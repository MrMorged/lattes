...
import com.uft.lattes.model.ArtigoPeriodico;
 ...
 * @author igor
 */
public class ArtigoPeriodicoRepository extends GenericRepository<ArtigoPeriodico>{

    public ArtigoPeriodicoRepository(EntityManager anEntityManager) {
    ...
    public List buscaInformacao(String palavra) {
        if (palavra != null) {
            ...
            QueryBuilder b = fullTextEntityManager.getSearchFactory().buildQueryBuilder().forEntity(ArtigoPeriodico.class).get();
            org.apache.lucene.search.Query luceneQuery = b.phrase().
                    onField("titulo").
                    boostedTo(3).
                    sentence(palavra).createQuery();
            javax.persistence.Query fullTextQuery = fullTextEntityManager.createFullTextQuery(luceneQuery, ArtigoPeriodico.class);
    ...
    public void index() {
        ...
        try {
            fullTextEntityManager.createIndexer(ArtigoPeriodico.class)
            ...
            ex.printStackTrace();
            Logger.getLogger(ArtigoPeriodicoRepository.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public List<Membro> BuscaMembroPorId(List<ArtigoPeriodico> listaIdLattes) {
        Long auxId;
        int i;
    ...
    /** 
     * Busca todos os artigos periódicos da UFT
     * faz a mesma coisa da função logo abaixo, porém com programação ao invés de SQL puro
     */
//    public List<ArtigoPeriodico> BuscaArtigoPeriodicoDaUft(){
//        List<ArtigoPeriodico> selecao = new ArrayList<ArtigoPeriodico>();
//        List<AtuacaoProfissional> resultadoAP;
//        int anoEntrada,anoSaida,anoArtigoPeriodico;
//        Query query = this.entityManager.createQuery("SELECT x FROM ArtigoPeriodico x");
//        List<ArtigoPeriodico> resultadoTT = query.getResultList();
//        for(int i=0; i< resultadoTT.size(); i++){
//            ArtigoPeriodico aux = resultadoTT.get(i);
...
//                anoArtigoPeriodico = Integer.parseInt(aux.getAno());
...
//                }
//                if(anoArtigoPeriodico >= anoEntrada && anoArtigoPeriodico <= anoSaida){
//                    selecao.add(aux);
//                    break;
//                }
//            }
...
//    }
    ...
    /**
     * Método para buscar os artigos periódicos que os professores produziram na UFT
     * faz o mesmo que a função logo acima
     */
    public List<ArtigoPeriodico> BuscaArtigoPeriodicoMaisRapido(){
        Query query = this.entityManager.createNativeQuery("select * "
                + "from artigo_periodico as T "
                ...
                + "else CAST(T.ano as numeric) <= CAST(P.ano_saida as numeric) end;", ArtigoPeriodico.class);
