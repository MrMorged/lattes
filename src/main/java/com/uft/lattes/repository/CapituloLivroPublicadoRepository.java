...
import com.uft.lattes.model.CapituloLivroPublicado;
 ...
 * @author igor
 */
public class CapituloLivroPublicadoRepository extends GenericRepository<CapituloLivroPublicado>{

    public CapituloLivroPublicadoRepository(EntityManager anEntityManager) {
    ...
    public List buscaInformacao(String palavra) {
        if (palavra != null) {
            ...
            QueryBuilder b = fullTextEntityManager.getSearchFactory().buildQueryBuilder().forEntity(CapituloLivroPublicado.class).get();
            org.apache.lucene.search.Query luceneQuery = b.phrase().
                    onField("titulo").
                    boostedTo(2).
                    sentence(palavra).createQuery();
            javax.persistence.Query fullTextQuery = fullTextEntityManager.createFullTextQuery(luceneQuery, CapituloLivroPublicado.class);
    ...
    public void index() {
        ...
        try {
            fullTextEntityManager.createIndexer(CapituloLivroPublicado.class)
            ...
            ex.printStackTrace();
            Logger.getLogger(CapituloLivroPublicadoRepository.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public List<Membro> BuscaMembroPorId(List<CapituloLivroPublicado> listaIdLattes) {
        Long auxId;
        int i;
    ...
    /** 
     * Busca todos os capitulos de livros publicados da UFT
     * faz a mesma coisa da função logo abaixo, porém com programação ao invés de SQL puro
     */
//    public List<CapituloLivroPublicado> BuscaCapituloLivroPublicadoDaUft(){
//        List<CapituloLivroPublicado> selecao = new ArrayList<CapituloLivroPublicado>();
//        List<AtuacaoProfissional> resultadoAP;
//        int anoEntrada,anoSaida,anoCapituloLivroPublicado;
//        Query query = this.entityManager.createQuery("SELECT x FROM CapituloLivroPublicado x");
//        List<CapituloLivroPublicado> resultadoTT = query.getResultList();
//        for(int i=0; i< resultadoTT.size(); i++){
//            CapituloLivroPublicado aux = resultadoTT.get(i);
...
//                anoCapituloLivroPublicado = Integer.parseInt(aux.getAno());
...
//                }
//                if(anoCapituloLivroPublicado >= anoEntrada && anoCapituloLivroPublicado <= anoSaida){
//                    selecao.add(aux);
//                    break;
//                }
//            }
...
//    }
    ...
    /**
     * Método para buscar os capitulos de livros publicados que os professores produziram na UFT
     * faz o mesmo que a função logo acima
     */
    public List<CapituloLivroPublicado> BuscaCapituloLivroPublicadoMaisRapido(){
        Query query = this.entityManager.createNativeQuery("select * "
                + "from capitulo_livro_publicado as T "
                ...
                + "else CAST(T.ano as numeric) <= CAST(P.ano_saida as numeric) end;", CapituloLivroPublicado.class);
