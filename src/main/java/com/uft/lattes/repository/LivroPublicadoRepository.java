...
import com.uft.lattes.model.LivroPublicado;
 ...
 * @author igor
 */
public class LivroPublicadoRepository extends GenericRepository<LivroPublicado> {

    public LivroPublicadoRepository(EntityManager anEntityManager) {
    ...
    public List buscaInformacao(String palavra) {
        if (palavra != null) {
            ...
            QueryBuilder b = fullTextEntityManager.getSearchFactory().buildQueryBuilder().forEntity(LivroPublicado.class).get();
            org.apache.lucene.search.Query luceneQuery = b.phrase().
                    onField("titulo").
                    boostedTo(2).
                    sentence(palavra).createQuery();
            javax.persistence.Query fullTextQuery = fullTextEntityManager.createFullTextQuery(luceneQuery, LivroPublicado.class);
    ...
    public void index() {
        ...
        try {
            fullTextEntityManager.createIndexer(LivroPublicado.class)
            ...
            ex.printStackTrace();
            Logger.getLogger(LivroPublicadoRepository.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public List<Membro> BuscaMembroPorId(List<LivroPublicado> listaIdLattes) {
        Long auxId;
        int i;
    ...
    /** 
     * Busca todos os livros publicados da UFT
     * faz a mesma coisa da função logo abaixo, porém com programação ao invés de SQL puro
     */
//    public List<LivroPublicado> BuscaLivroPublicadoDaUft(){
//        List<LivroPublicado> selecao = new ArrayList<LivroPublicado>();
//        List<AtuacaoProfissional> resultadoAP;
//        int anoEntrada,anoSaida,anoLivroPublicado;
//        Query query = this.entityManager.createQuery("SELECT x FROM LivroPublicado x");
//        List<LivroPublicado> resultadoTT = query.getResultList();
//        for(int i=0; i< resultadoTT.size(); i++){
//            LivroPublicado aux = resultadoTT.get(i);
...
//                anoLivroPublicado = Integer.parseInt(aux.getAno());
...
//                }
//                if(anoLivroPublicado >= anoEntrada && anoLivroPublicado <= anoSaida){
//                    selecao.add(aux);
//                    break;
//                }
//            }
...
//    }
     ...
     */
    public List<LivroPublicado> BuscaLivroPublicadoMaisRapido(){
        Query query = this.entityManager.createNativeQuery("select * "
                + "from livro_publicado as T "
                ...
                + "else CAST(T.ano as numeric) <= CAST(P.ano_saida as numeric) end;", LivroPublicado.class);
