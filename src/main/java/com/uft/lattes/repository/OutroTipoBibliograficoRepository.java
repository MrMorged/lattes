...
import com.uft.lattes.model.OutroTipoBibliografico;
 ...
 * @author igor
 */
public class OutroTipoBibliograficoRepository extends GenericRepository<OutroTipoBibliografico>{

    public OutroTipoBibliograficoRepository(EntityManager anEntityManager) {
    ...
    public List buscaInformacao(String palavra) {
        if (palavra != null) {
            ...
            QueryBuilder b = fullTextEntityManager.getSearchFactory().buildQueryBuilder().forEntity(OutroTipoBibliografico.class).get();
            org.apache.lucene.search.Query luceneQuery = b.phrase().
                    onField("titulo").
                    boostedTo(3).
                    sentence(palavra).createQuery();
            javax.persistence.Query fullTextQuery = fullTextEntityManager.createFullTextQuery(luceneQuery, OutroTipoBibliografico.class);
    ...
    public void index() {
        ...
        try {
            fullTextEntityManager.createIndexer(OutroTipoBibliografico.class)
            ...
            ex.printStackTrace();
            Logger.getLogger(OutroTipoBibliograficoRepository.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public List<Membro> BuscaMembroPorId(List<OutroTipoBibliografico> listaIdLattes) {
        Long auxId;
        int i;
    ...
    /** 
     * Busca todos os outros tipos bibliograficos da UFT
     * faz a mesma coisa da função logo abaixo, porém com programação ao invés de SQL puro
     */
//    public List<OutroTipoBibliografico> BuscaOutroTipoBibliograficoDaUft(){
//        List<OutroTipoBibliografico> selecao = new ArrayList<OutroTipoBibliografico>();
//        List<AtuacaoProfissional> resultadoAP;
//        int anoEntrada,anoSaida,anoOutroTipoBibliografico;
//        Query query = this.entityManager.createQuery("SELECT x FROM OutroTipoBibliografico x");
//        List<OutroTipoBibliografico> resultadoTT = query.getResultList();
//        for(int i=0; i< resultadoTT.size(); i++){
//            OutroTipoBibliografico aux = resultadoTT.get(i);
...
//                anoOutroTipoBibliografico = Integer.parseInt(aux.getAno());
...
//                }
//                if(anoOutroTipoBibliografico >= anoEntrada && anoOutroTipoBibliografico <= anoSaida){
//                    selecao.add(aux);
//                    break;
//                }
//            }
...
//    }
    ...
    /**
     * Método para buscar os outros tipos bibliograficos que os professores produziram na UFT
     * faz o mesmo que a função logo acima
     */
    public List<OutroTipoBibliografico> BuscaOutroTipoBibliograficoMaisRapido(){
        Query query = this.entityManager.createNativeQuery("select * "
                + "from outro_tipo_bibliografico as T "
                ...
                + "else CAST(T.ano as numeric) <= CAST(P.ano_saida as numeric) end;", OutroTipoBibliografico.class);
