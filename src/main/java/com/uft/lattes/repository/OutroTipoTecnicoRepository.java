...
import com.uft.lattes.model.OutroTipoTecnico;
 ...
 * @author igor
 */
public class OutroTipoTecnicoRepository extends GenericRepository<OutroTipoTecnico> {

    public OutroTipoTecnicoRepository(EntityManager entityManager) {
        super(entityManager);
    }
    ...
    public List buscaInformacao(String palavra) {
        if (palavra != null) {
            ...
            QueryBuilder b = fullTextEntityManager.getSearchFactory().buildQueryBuilder().forEntity(OutroTipoTecnico.class).get();
            org.apache.lucene.search.Query luceneQuery = b.phrase().
                    onField("titulo").
                    boostedTo(3).
                    sentence(palavra).createQuery();
            javax.persistence.Query fullTextQuery = fullTextEntityManager.createFullTextQuery(luceneQuery, OutroTipoTecnico.class);
    ...
    public void index() {
        ...
        try {
            fullTextEntityManager.createIndexer(OutroTipoTecnico.class)
            ...
            ex.printStackTrace();
            Logger.getLogger(OutroTipoTecnicoRepository.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public List<Membro> BuscaMembroPorId(List<OutroTipoTecnico> listaIdLattes) {
        Long auxId;
        int i;
    ...
    /** 
     * Busca todos os trabalhos técnicos da UFT
     * faz a mesma coisa da função logo abaixo, porém com programação ao invés de SQL puro
     */
//    public List<OutroTipoTecnico> BuscaOutroTipoTecnicoDaUft(){
//        List<OutroTipoTecnico> selecao = new ArrayList<OutroTipoTecnico>();
//        List<AtuacaoProfissional> resultadoAP;
//        int anoEntrada,anoSaida,anoOutroTipoTecnico;
//        Query query = this.entityManager.createQuery("SELECT x FROM OutroTipoTecnico x");
//        List<OutroTipoTecnico> resultadoTT = query.getResultList();
//        for(int i=0; i< resultadoTT.size(); i++){
//            OutroTipoTecnico aux = resultadoTT.get(i);
...
//                anoOutroTipoTecnico = Integer.parseInt(aux.getAno());
...
//                }
//                if(anoOutroTipoTecnico >= anoEntrada && anoOutroTipoTecnico <= anoSaida){
//                    selecao.add(aux);
//                    break;
//                }
//            }
...
//    }
    ...
    /**
     * Método para buscar os trabalhos técnicos que os professores produziram na UFT
     * faz o mesmo que a função logo acima
     */
    public List<OutroTipoTecnico> BuscaOutroTipoTecnicoMaisRapido(){
        Query query = this.entityManager.createNativeQuery("select * "
                + "from outro_tipo_tecnico as T "
                ...
                + "else CAST(T.ano as numeric) <= CAST(P.ano_saida as numeric) end;", OutroTipoTecnico.class);
