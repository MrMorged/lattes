...
import com.uft.lattes.model.SoftwareComPatente;
 ...
 * @author igor
 */
public class SoftwareComPatenteRepository extends GenericRepository<SoftwareComPatente>{
    
    public SoftwareComPatenteRepository(EntityManager entityManager){
        super(entityManager);
    }
    ...
    public List buscaInformacao(String palavra) {
        if (palavra != null) {
            ...
            QueryBuilder b = fullTextEntityManager.getSearchFactory().buildQueryBuilder().forEntity(SoftwareComPatente.class).get();
            org.apache.lucene.search.Query luceneQuery = b.phrase().
                    onField("titulo").
                    boostedTo(3).
                    sentence(palavra).createQuery();
            javax.persistence.Query fullTextQuery = fullTextEntityManager.createFullTextQuery(luceneQuery, SoftwareComPatente.class);
    ...
    public void index() {
        ...
        try {
            fullTextEntityManager.createIndexer(SoftwareComPatente.class)
            ...
            ex.printStackTrace();
            Logger.getLogger(SoftwareComPatenteRepository.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public List<Membro> BuscaMembroPorId(List<SoftwareComPatente> listaIdLattes) {
        Long auxId;
        int i;
    ...
    /** 
     * Busca todos os softwares com patentes da UFT
     * faz a mesma coisa da função logo abaixo, porém com programação ao invés de SQL puro
     */
//    public List<SoftwareComPatente> BuscaSoftwareComPatenteDaUft(){
//        List<SoftwareComPatente> selecao = new ArrayList<SoftwareComPatente>();
//        List<AtuacaoProfissional> resultadoAP;
//        int anoEntrada,anoSaida,anoSoftwareComPatente;
//        Query query = this.entityManager.createQuery("SELECT x FROM SoftwareComPatente x");
//        List<SoftwareComPatente> resultadoTT = query.getResultList();
//        for(int i=0; i< resultadoTT.size(); i++){
//            SoftwareComPatente aux = resultadoTT.get(i);
...
//                anoSoftwareComPatente = Integer.parseInt(aux.getAno());
...
//                }
//                if(anoSoftwareComPatente >= anoEntrada && anoSoftwareComPatente <= anoSaida){
//                    selecao.add(aux);
//                    break;
//                }
//            }
...
//    }
    ...
    /**
     * Método para buscar os softwares com patentes que os professores produziram na UFT
     * faz o mesmo que a função logo acima
     */
    public List<SoftwareComPatente> BuscaSoftwareComPatenteMaisRapido(){
        Query query = this.entityManager.createNativeQuery("select * "
                + "from software_com_patente as T "
                ...
                + "else CAST(T.ano as numeric) <= CAST(P.ano_saida as numeric) end;", SoftwareComPatente.class);
