...
import com.uft.lattes.model.TextoJornalNoticia;
 ...
 * @author igor
 */
public class TextoJornalNoticiaRepository extends GenericRepository<TextoJornalNoticia>{

    public TextoJornalNoticiaRepository(EntityManager anEntityManager) {
    ...
    public List buscaInformacao(String palavra) {
        if (palavra != null) {
            ...
            QueryBuilder b = fullTextEntityManager.getSearchFactory().buildQueryBuilder().forEntity(TextoJornalNoticia.class).get();
            org.apache.lucene.search.Query luceneQuery = b.phrase().
                    onField("titulo").
                    boostedTo(3).
                    sentence(palavra).createQuery();
            javax.persistence.Query fullTextQuery = fullTextEntityManager.createFullTextQuery(luceneQuery, TextoJornalNoticia.class);
    ...
    public void index() {
        ...
        try {
            fullTextEntityManager.createIndexer(TextoJornalNoticia.class)
            ...
            ex.printStackTrace();
            Logger.getLogger(TextoJornalNoticiaRepository.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public List<Membro> BuscaMembroPorId(List<TextoJornalNoticia> listaIdLattes) {
        Long auxId;
        int i;
        ...
        /** 
     * Busca todos os textos em jornais ou noticias da UFT
     * faz a mesma coisa da função logo abaixo, porém com programação ao invés de SQL puro
     */
//    public List<TextoJornalNoticia> BuscaTextoJornalNoticiaDaUft(){
//        List<TextoJornalNoticia> selecao = new ArrayList<TextoJornalNoticia>();
//        List<AtuacaoProfissional> resultadoAP;
//        int anoEntrada,anoSaida,anoTextoJornalNoticia;
//        Query query = this.entityManager.createQuery("SELECT x FROM TextoJornalNoticia x");
//        List<TextoJornalNoticia> resultadoTT = query.getResultList();
//        for(int i=0; i< resultadoTT.size(); i++){
//            TextoJornalNoticia aux = resultadoTT.get(i);
...
//                anoTextoJornalNoticia = Integer.parseInt(aux.getAno());
...
//                }
//                if(anoTextoJornalNoticia >= anoEntrada && anoTextoJornalNoticia <= anoSaida){
//                    selecao.add(aux);
//                    break;
//                }
//            }
...
//    }
    ...
    /**
     * Método para buscar os texto em jornais ou noticias que os professores produziram na UFT
     * faz o mesmo que a função logo acima
     * 
     * Essa funcao tem caso especial, pois nem todos tem o ano cadastrado, por isso o CASE T.ano= ''
     * quando for verdadeiro, usei zero(0) como ano, somente para desconsiderar, dessa forma não será contado como publicação,
     * pois não tem como saber se o professor trabalhava na UFT quando produziu
     */
    public List<TextoJornalNoticia> BuscaTextoJornalNoticiaMaisRapido(){
        Query query = this.entityManager.createNativeQuery("select * "
                + "from texto_jornal_noticia as T "
                ...
                + "CASE when T.ano = '' then 0 >= cast(P.ano_entrada as numeric) else "
                + "cast(T.ano as numeric) >= cast(P.ano_entrada as numeric) end"
                + "AND CASE when P.ano_saida = 'Atual' THEN EXTRACT(YEAR FROM CURRENT_TIMESTAMP) >= CAST (T.ano as numeric) "
                + "else CAST(T.ano as numeric) <= CAST(P.ano_saida as numeric) end;", TextoJornalNoticia.class);
