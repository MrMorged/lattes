...
import com.uft.lattes.model.TrabalhoCompletoCongresso;
 ...
 * @author igor
 */
public class TrabalhoCompletoCongressoRepository extends GenericRepository<TrabalhoCompletoCongresso>{

    public TrabalhoCompletoCongressoRepository(EntityManager anEntityManager) {
    ...
    public List buscaInformacao(String palavra) {
        if (palavra != null) {
            ...
            QueryBuilder b = fullTextEntityManager.getSearchFactory().buildQueryBuilder().forEntity(TrabalhoCompletoCongresso.class).get();
            org.apache.lucene.search.Query luceneQuery = b.phrase().
                    onField("titulo").
                    boostedTo(3).
                    sentence(palavra).createQuery();
            javax.persistence.Query fullTextQuery = fullTextEntityManager.createFullTextQuery(luceneQuery, TrabalhoCompletoCongresso.class);
    ...
    public void index() {
        ...
        try {
            fullTextEntityManager.createIndexer(TrabalhoCompletoCongresso.class)
            ...
            ex.printStackTrace();
            Logger.getLogger(TrabalhoCompletoCongressoRepository.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public List<Membro> BuscaMembroPorId(List<TrabalhoCompletoCongresso> listaIdLattes) {
        Long auxId;
        int i;
    ...
    /** 
     * Busca todos os trabalhos completos em congressos da UFT
     * faz a mesma coisa da função logo abaixo, porém com programação ao invés de SQL puro
     */
//    public List<TrabalhoCompletoCongresso> BuscaTrabalhoCompletoCongressoDaUft(){
//        List<TrabalhoCompletoCongresso> selecao = new ArrayList<TrabalhoCompletoCongresso>();
//        List<AtuacaoProfissional> resultadoAP;
//        int anoEntrada,anoSaida,anoTrabalhoCompletoCongresso;
//        Query query = this.entityManager.createQuery("SELECT x FROM TrabalhoCompletoCongresso x");
//        List<TrabalhoCompletoCongresso> resultadoTT = query.getResultList();
//        for(int i=0; i< resultadoTT.size(); i++){
//            TrabalhoCompletoCongresso aux = resultadoTT.get(i);
...
//                anoTrabalhoCompletoCongresso = Integer.parseInt(aux.getAno());
...
//                }
//                if(anoTrabalhoCompletoCongresso >= anoEntrada && anoTrabalhoCompletoCongresso <= anoSaida){
//                    selecao.add(aux);
//                    break;
//                }
//            }
...
//    }
    ...
    /**
     * Método para buscar os trabalhos completos em congressos que os professores produziram na UFT
     * faz o mesmo que a função logo acima
     */
    public List<TrabalhoCompletoCongresso> BuscaTrabalhoCompletoCongressoMaisRapido(){
        Query query = this.entityManager.createNativeQuery("select * "
                + "from trabalho_completo_congresso as T "
                ...
                + "else CAST(T.ano as numeric) <= CAST(P.ano_saida as numeric) end;", TrabalhoCompletoCongresso.class);
    ...
    public List getSeparaPorAno(long id){
        Query query = this.entityManager.createNativeQuery("select count(ano), T.ano "
                ...
                + "inner join atuacao_profissional as P on T.membro_id_lattes = P.membro_id_lattes "
                + "inner join membro as M on T.membro_id_lattes = M.id_lattes "
                + "where P.sigla = 'UFT' AND id_curso = :id AND "
                + "cast(T.ano as numeric) >= cast(P.ano_entrada as numeric) "
