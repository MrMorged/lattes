...
/* 
 * Busca todas as produções técnicas, dividindo em grupos por ano
 */
public class TrabalhoTecnicoRepository extends GenericRepository<TrabalhoTecnico>{
    
    public TrabalhoTecnicoRepository(EntityManager entityManager){
        super(entityManager);
    }
    ...
    public List buscaInformacao(String palavra) {
        if (palavra != null) {
            ...
            QueryBuilder b = fullTextEntityManager.getSearchFactory().buildQueryBuilder().forEntity(TrabalhoTecnico.class).get();
            org.apache.lucene.search.Query luceneQuery = b.phrase().
                    onField("titulo").
                    boostedTo(3).
                    sentence(palavra).createQuery();
            javax.persistence.Query fullTextQuery = fullTextEntityManager.createFullTextQuery(luceneQuery, TrabalhoTecnico.class);
    ...
    public void index() {
        ...
        try {
            fullTextEntityManager.createIndexer(TrabalhoTecnico.class)
            ...
            ex.printStackTrace();
            Logger.getLogger(TrabalhoTecnicoRepository.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    
    public List<Membro> BuscaMembroPorId(List<TrabalhoTecnico> listaIdLattes) {
        Long auxId;
        int i;
     ...
     */
//    public List<TrabalhoTecnico> BuscaTrabalhoTecnicoDaUft(){
//        List<TrabalhoTecnico> selecao = new ArrayList<TrabalhoTecnico>();
//        List<AtuacaoProfissional> resultadoAP;
//        int anoEntrada,anoSaida,anoTrabalhoTecnico;
//        Query query = this.entityManager.createQuery("SELECT x FROM TrabalhoTecnico x");
//        List<TrabalhoTecnico> resultadoTT = query.getResultList();
//        for(int i=0; i< resultadoTT.size(); i++){
//            TrabalhoTecnico aux = resultadoTT.get(i);
...
//                anoTrabalhoTecnico = Integer.parseInt(aux.getAno());
...
//                }
//                if(anoTrabalhoTecnico >= anoEntrada && anoTrabalhoTecnico <= anoSaida){
//                    selecao.add(aux);
//                    break;
//                }
//            }
...
//    }
     ...
     */
    public List<TrabalhoTecnico> BuscaTrabalhoTecnicoMaisRapido(){
        Query query = this.entityManager.createNativeQuery("select * "
                + "from trabalho_tecnico as T "
                ...
                + "else CAST(T.ano as numeric) <= CAST(P.ano_saida as numeric) end;", TrabalhoTecnico.class);
